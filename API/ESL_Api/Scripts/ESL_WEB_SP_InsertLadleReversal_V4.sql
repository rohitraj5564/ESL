CREATE OR ALTER PROCEDURE [dbo].[ESL_WEB_SP_InsertLadleReversal_V4]
(
    @ID UNIQUEIDENTIFIER,
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRAN
            ...
            SELECT @movementTypeID = TypeID 
            FROM MovementType 
            WHERE TypeDescription = @movementType

            IF @movementTypeID IS NULL
            BEGIN
                SELECT TOP 1 @movementTypeID = RL.MovementTypeID
                FROM   ReversalLadles RL
                WHERE  RL.LadleNo  = @ladleNo
                  AND  RL.IsActive = 1
                  AND  RL.IsBooked = 1
                  AND  RL.CompletedDateTime IS NULL
                ORDER BY RL.BookingDateTime DESC;
            END

            IF @movementTypeID IS NULL
            BEGIN
                SET @movementTypeID = 6;
            END
            ...
            BEGIN
                ...
                (
                    ID, LadleMovementID, CastID, LadleNo, MovementTypeID,
                    SourceLocationID, DestinationLocationID, IsActive,
                    CreatedDateTime, ServerDateTime
                )
                VALUES 
                (
                    @ID, @ladleMovementID, @castID, @ladleNo, @movementTypeID,
                    @sourceID, @destinationID, 1, @transactionDateTime, GETDATE()
                )
                ...
                WHERE LadleNo = @ladleNo 
                  AND IsActive = 1
                  AND MovementTypeID NOT IN (2, 3, 4, 6)  
    ...
    BEGIN CATCH
        ...
        RAISERROR('ESL_WEB_SP_InsertLadleReversal_V4 failed at line %d: %s', 16, 1, @ErrLine, @ErrMsg);
    END CATCH
    RETURN @status
END
GO
